...
import { EXPENSE_ICONS } from "../../hooks/useTransactions";
...
type Props = {
    ...
    setCustomExpense: (v: string) => void;
    onOpenDropdown: () => void;
    onAdd: () => void;
};

export default function ExpenseForm({ expenseInput, setExpenseInput, selectedCategory, customExpense, setCustomExpense, onOpenDropdown, onAdd }: Props) {
    const { colors } = useTheme();

    return (
        <View style={[styles.inputCard, { backgroundColor: colors.card }]}>
            <View style={styles.inputCardHeader}>
                <View style={[styles.inputCardIcon, { backgroundColor: "rgba(239,68,68,0.15)" }]}>
                    <Ionicons name="arrow-up-circle" size={22} color={colors.danger} />
                </View>
                <Text style={[styles.inputCardTitle, { color: colors.text }]}>Dodaj trošak</Text>
            </View>
            ...
            <Pressable style={[styles.dropdownTrigger, { backgroundColor: colors.input }]} onPress={onOpenDropdown}>
                <View style={styles.dropdownLeft}>
                    <Ionicons name={(EXPENSE_ICONS[selectedCategory] || "receipt") as any} size={16} color={colors.danger} />
                    <Text style={[styles.dropdownText, { color: colors.text }]}>{selectedCategory}</Text>
                </View>
                <Ionicons name="chevron-down" size={18} color={colors.textMuted} />
            </Pressable>

            {selectedCategory === "Ostalo (Upiši sam)" && (
                <TextInput
            ...
            <Pressable style={[styles.primaryBtn, { backgroundColor: colors.danger }]} onPress={onAdd}>
                <Ionicons name="remove-circle" size={18} color="#fff" />
                <Text style={styles.primaryBtnText}>Dodaj trošak</Text>
            </Pressable>
        </View>
    );
}
